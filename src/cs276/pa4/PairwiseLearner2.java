...
public class PairwiseLearner2 extends PointwiseLearner {
  private LibSVM model;
  public PairwiseLearner2(boolean isLinearKernel){
    try{
      model = new LibSVM();
    } catch (Exception e){
  ...
  public PairwiseLearner2(double C, double gamma, boolean isLinearKernel){
    try{
      model = new LibSVM();
    } catch (Exception e){
      e.printStackTrace();
  ...
  public static Instances standardizeInstance(Instances X) {
	...
	public static Instances createInstancesTemplate () {
		...
		attributes.add(new Attribute("bm25score"));
			...
			Map<String, Double> idfs) 
	{
		...
		relData = Util.loadRelData(rel_file);
		 queryDict = LoadHandler.loadTrainData(train_data_file);
		}catch(Exception e){
			System.out.println(e.getMessage());
			throw new RuntimeException();
		}
		
		BM25Scorer scorer = new BM25Scorer(idfs, queryDict);
		...
		for(Query q: trainingDocData.keySet()){
			...
			for(Document doc: testDocs){
				...
				double[] instance = {atts.get("url"), atts.get("title"), atts.get("body"), atts.get("header"), atts.get("anchor"), atts.get("bm25score"),atts.get("pagerank"),atts.get("window"),relData.get(queryString).get(doc.url)};
			...
			Map<String, Double> idfs) 
	{
		...
		Map<Query, Map<String, Document>> queryDocs = null;
		try{
   		 testDocData = Util.loadTrainData(test_data_file);
		 queryDocs = LoadHandler.loadTrainData(test_data_file);
		}catch(Exception e){
			e.printStackTrace(); throw new RuntimeException();
		}
		BM25Scorer scorer = new BM25Scorer(idfs,queryDocs);
		...
		for(Query q: testDocData.keySet()){
			...
			for(Document doc: testDocs){
				...
				double[] instance = {atts.get("url"), atts.get("title"), atts.get("body"), atts.get("header"), atts.get("anchor"),atts.get("bm25score"),atts.get("pagerank"),atts.get("window"), 0};
				Instance inst = new DenseInstance(1.0, instance); 
				ctr++;
				X.add(inst);
				Map<String, Integer> map = t.indexMap.get(queryString);
