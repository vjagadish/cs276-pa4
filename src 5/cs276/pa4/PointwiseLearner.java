...
public class PointwiseLearner extends Learner {
	...
	public TestFeatures extract_test_features(String test_data_file,
		...
		featStore.index_map = new HashMap<String, Map<String, Integer>>();
		CosineSimilarityScorer instanceBuilder = new CosineSimilarityScorer(idfs);
		Integer count = 0;
		...
		for(Query q: testQueryDocs.keySet()){
			...
			featStore.index_map.put(queryString, index);
		}
	...
	public Map<String, List<String>> testing(TestFeatures tf,
		...
		{	//System.out.println(query);
			...
			List<Pair<String,Double>> urlAndScores = new ArrayList<Pair<String,Double>>(tf.index_map.get(query).size());
			for (String url : tf.index_map.get(query).keySet())
			{	
				//System.out.print(url+"  ");
				double score = 0.0;
				try{
					score = model.classifyInstance(tf.features.get(tf.index_map.get(query).get(url)));
